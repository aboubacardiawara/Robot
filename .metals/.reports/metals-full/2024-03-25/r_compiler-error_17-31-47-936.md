...
public class HunterSecondary extends SecondaryBotBaseBrain {
  ...
  protected IState buildStateMachine() {
    ...
    IState STTurnWestSup = new State();
    STTurnWestSup.setDescription("Turn West Sup");
    MoveStateTORedect.setDescription("Move State To Redect");
    IState STTurnWestSRDOWN = new State();
    STTurnWestSRDOWN.setDescription("Turn West SRDOWN");
    ...
    DetecState.addNext(STTurnWestSup, () -> detectWall() && isSameDirection(getHeading(), Parameters.EAST) && this.currentRobot == Robots.SRUP);
    DetecState.addNext(STTurnWestSRDOWN, () -> detectWall() && isSameDirection(getHeading(), Parameters.EAST) && this.currentRobot == Robots.SRBOTTOM);
    ...
    DetecState.addNext(STTurnWestSup, () -> detectWall() && isSameDirection(getHeading(), Parameters.WEST));
    STTurnWestSup.addNext(DetecState, () -> isSameDirection(getHeading(),  -Math.PI) );
    STTurnWestSup.setStateAction(() -> { turnLeft(); });
    STTurnWestSRDOWN.addNext(DetecState, () -> isSameDirection(getHeading(),  3*Math.PI/2) );
    STTurnWestSRDOWN.setStateAction(() ->{turnLeft();});
  ...
  private String buildOpponentPosMessage(IRadarResult radarResult, double opponen@@tPosX, double opponentPosY) {
