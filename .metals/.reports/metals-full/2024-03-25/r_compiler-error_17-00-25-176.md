...
public class HunterSecondary extends SecondaryBotBaseBrain {
  ...
  @@  // Detecting wall and deciding the next state based on the robot's heading direction
  DetecState.addNext(STTurnWest, () -> detectWall() && (isSameDirection(getHeading(), Parameters.EAST) || isSameDirection(getHeading(), Parameters.NORTH)));
  DetecState.addNext(STTurnEast, () -> detectWall() && (isSameDirection(getHeading(), Parameters.SOUTH) || isSameDirection(getHeading(), Parameters.WEST)));

  // Defining actions for STTurnWest state
  STTurnWest.addNext(DetecState, () -> isSameDirection(getHeading(), 3*Math.PI/2));
  STTurnWest.setStateAction(() -> {
      // Assuming turnLeft() turns the robot 90 degrees to the left
      turnLeft();
  });

  // Defining actions for STTurnEast state
  STTurnEast.addNext(DetecState, () -> isSameDirection(getHeading(), Math.PI/2));
  STTurnEast.setStateAction(() -> {
      // Assuming turnRight() turns the robot 90 degrees to the right
      turnRight();
  });
