...
// Adjusting the condition for turning right when facing north and detecting a wall
DetecState.addNext(STTurnEast, () -> detectWall() && isSameDirection(getHeading(), Parameters.NORTH));

// Keep the existing conditions for other directions
DetecState.addNext(STTurnWest, () -> detectWall() && isSameDirection(getHeading(), Parameters.EAST));
DetecState.addNext(STTurnWest, () -> detectWall() && (isSameDirection(getHeading(), Parameters.NORTH) || isSameDirection(getHeading(), Parameters.WEST)));
DetecState.addNext(STTurnEast, () -> detectWall() && isSameDirection(getHeading(), Parameters.SOUTH));

// Actions for turning west (left turn) when in STTurnWest state
STTurnWest.addNext(DetecState, () -> isSameDirection(getHeading(), Math.PI/2));
STTurnWest.setStateAction(() -> {
    turnLeft(); // Ensure this turns the robot 90 degrees left
});

// Actions for turning east (right turn) when in STTurnEast state
STTurnEast.addNext(DetecState, () -> isSameDirection(get@@Heading(), Math.PI/2));
STTurnEast.setStateAction(() -> {
  turnLeft(); // Ensure this turns the robot 90 degrees right
});
