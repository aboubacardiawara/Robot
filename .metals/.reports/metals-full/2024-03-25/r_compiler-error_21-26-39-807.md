...
public class HunterSecondary extends SecondaryBotBaseBrain {
  ...
  private int move_back_count = 50;
  private boolean bullet_detected = false;
  double newDirection  = -1;
  ...
  protected IState buildStateMachine() {
    ...
    DetecState.setStateAction(() -> {
      ...
      newDirection = getHeading() + rotationAngle;
  ...
  // Deblocage quand je detect un autre robot devant moi.
    DetecState.addNext(DeblocState, () -> opponentFrontOfMe() );
    DeblocState.addNext(DetecState, () -> isSameDirection(getHeading(), newDirection));
    DeblocState.setStateAction(() -> {
      turnRight();
    });
    return initState;
  }
 ...
 radar.getObjectType() == IRadarResult.Types.OpponentSecondaryBot) {
        ...
        if (enemyAbsoluteAngle >= normalize(this.getHeading() - Math.PI / 4)
                && enemyAbsoluteAngle <= normalize(this.getHeading() + Math.PI / 4)) {
            return true;
        }
